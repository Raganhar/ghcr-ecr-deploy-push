...
class StandardPulumiEcs : Stack
{
    public StandardPulumiEcs()
    {
        ...
        var webSg = new Ec2.SecurityGroup($"{serviceName}-sg", new Ec2.SecurityGroupArgs
        {
            VpcId = vpcId,
            Egress =
            {
        ...
        var cluster = new Ecs.Cluster($"{serviceName}-cluster");
        ...
        var webLb = new Elb.LoadBalancer($"{serviceName}-web-lb", new Elb.LoadBalancerArgs
        {
            Subnets = subnetIds,
            SecurityGroups = { webSg.Id }
        });
        var webTg = new Elb.TargetGroup($"{serviceName}-web-tg", new Elb.TargetGroupArgs
        {
            Port = 80,
            Protocol = "HTTP",
            TargetType = "ip",
        ...
        var webListener = new Elb.Listener($"{serviceName}-web-listener", new Elb.ListenerArgs
        {
        ...
        var appRepo = new Ecr.Repository($"{serviceName}-repo");
        ...
        var image = new Docker.Image($"{serviceName}-img", new Docker.ImageArgs
        {
            Build = "../",
